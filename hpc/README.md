...
## Filling gaps left by a killed job

A walltime kill leaves a sweep partially complete. Do not resubmit the whole
sweep: at 2-D N=64 a single row is 38 hours.

```bash
python scripts/utils/gap_analysis.py --dim 2      # what is missing, and why
```

It writes `results/manifests/rerun_2d.json` and distinguishes a genuinely
missing row from one whose solution archive survived but whose summary row did
not. Feed its scope back through the same sweep script, restricted and
appending:

```bash
export N_VALUES="64"; export SOLVERS="qsvt"
qsub -v N_VALUES,SOLVERS hpc/jobs/submit_hpc_2D.sh
```

The sweep scripts already pass `--append`, so a restricted run merges into the
existing summary instead of replacing it. Invoking a runner by hand does not:
see "Partial results are safe" below. Rows whose `.npz` survived are cheaper
still, and `scripts/utils/recover_orphan_rows.py` reconstructs those without
recomputing anything.

**The N range is a variable, not a separate script.** `N_VALUES` overrides the
tier list and `LARGE_N` the second-phase QSVT tier, which already defaults to
`128,256` in 2-D and `32` in 3-D. The extended resolutions reported in the
thesis therefore come from the same sweep scripts with nothing set. To drive
the whole range through phase 1 instead, suppress phase 2 so it does not
recompute the top tiers:

```bash
export N_VALUES="4,8,16,32,64,128,256"; export SOLVERS="qsvt"; export SKIP_LARGE_N=1
qsub -v N_VALUES,SOLVERS,SKIP_LARGE_N hpc/jobs/submit_hpc_2D.sh
```
