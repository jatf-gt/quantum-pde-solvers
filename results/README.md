...
| `2Dstudies_vqls/` | A study still in flight, held apart until it completes. See its `PROVENANCE.md`. |
...
run tag together — `1Dstudies`, `1Dstudies_4th`, `2Dstudies_vqls` — because none
of the three is recoverable from the filenames within it. A tagged directory
holds a run that has not yet been merged into its untagged counterpart, either
because it is unfinished or because it has not been reviewed.
