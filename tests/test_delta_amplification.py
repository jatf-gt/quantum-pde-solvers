...
class TestDiscretizationErrorReference:
    ...
    def test_follows_h_squared_scaling(self, N):
        """
        Validates that the discretisation error converges at the expected second-order rate with grid refinement.
        Establishes the numerical baseline for subsequent perturbation analysis.
        """
    ...
    def test_error_decreases_monotonically_with_N(self):
        """
        Confirms that the continuous approximation uniformly improves as spatial resolution increases.
        Ensures standard convergence behaviour.
        """
        errors = []
        for N in (8, 16, 32, 64):
            prob, u_exact = analytic_problem(N)
...
class TestAmplificationMeasurement:
    ...
    def test_fmg_amplification_roughly_constant_across_N(self):
        """
        Validates that Full Multigrid iteration amplifies injected solver error uniformly across grid resolutions.
        Supports its proposed scalability.
        """
        delta = 0.005
    ...
    def test_sor_amplification_grows_with_N(self):
        """
        Confirms that Successive Over-Relaxation iteration exhibits problem-size-dependent error amplification.
        Highlights the contrast with multigrid stability.
        """
        delta = 0.005
        amps = [self._measure(N, delta, "sor") / delta for N in (8, 16, 32)]
        assert amps == sorted(amps)  # monotonically increasing
        assert amps[-1] / amps[0] > 2.0  # confirms macroscopic growth, not numerical noise

    def test_fmg_amplification_roughly_linear_in_delta(self):
        """
        Ensures that the error amplification from Full Multigrid scales linearly with the magnitude of the injected perturbation.
        Validates the structural assumptions of the perturbation model.
        """
    ...
    def test_sor_amplification_less_linear_than_fmg(self):
        """
        Validates that Successive Over-Relaxation shows more non-linear variation in its amplification profile than Full Multigrid.
        Demonstrates its relative proximity to stability limits.
        """
...
class TestDivergenceDetection:
    ...
    *lack* of improvement, not active divergence, and a smooth exponential
    ...
    def test_small_delta_is_not_flagged_as_diverged(self):
        """
        Confirms that minor perturbations allow the solver to converge stably without triggering divergence protections.
        Ensures robust operation in the expected regime.
        """
    ...
    def test_large_delta_fmg_diverges_at_N32(self):
        """
        Validates that Full Multigrid correctly triggers divergence detection under extreme perturbation at moderate resolution.
        Tests the boundary of solver stability.
        """
    ...
    def test_large_delta_sor_diverges_broadly(self):
        """
        Ensures that Successive Over-Relaxation predictably diverges under significant perturbation even at low grid resolutions.
        Confirms the expected narrow stability threshold.
        """
    ...
    def test_diverged_run_residual_is_not_merely_large_but_growing(self):
        """
        Validates that the divergence monitor distinguishes between a large but bounded stable residual and an actively growing unstable one.
        Confirms the accuracy of the detection heuristic.
        """
        # Distinguishes "large but stable" from "diverging": a stable run
        # at an elevated delta can yield a large absolute residual without its
        # ratio test firing. This confirms the two states are distinct
        # phenomena, rather than identical mechanisms separated by scale.
        prob, _u = analytic_problem(32)
...
class TestRowOperatorConditioning:
    ...
    def test_kappa_row_stays_bounded(self, Nx):
        """
        Confirms that the condition number of the row operator remains strictly bounded across resolutions.
        Validates the fundamental premise of the line-based decomposition strategy.
        """
    ...
    def test_kappa_row_does_not_grow_with_Nx(self):
        """
        Ensures that the row operator's condition number exhibits stability, contrasting with the quadratic growth seen in pure 1D Poisson operators.
        Corroborates the theoretical scaling advantage.
        """
        kappas = []
